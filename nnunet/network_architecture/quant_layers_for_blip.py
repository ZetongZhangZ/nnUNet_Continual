...
class Conv3d_Q(nn.Conv3d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        self.register_buffer('Fisher_w', torch.zeros_like(self.weight))
        self.register_buffer('Fisher_w_old', self.F_prior*torch.ones_like(self.weight))
        self.weight.data.normal_(0.0, math.sqrt(1.0/(in_channels*kernel_size*kernel_size*kernel_size)))
        self.weight.data.clamp_(-self.bound, self.bound)
        if self.bias is not None:
            self.register_buffer('prev_bias', torch.zeros_like(self.bias))
            self.register_buffer('bit_alloc_b', torch.zeros_like(self.bias, dtype=torch.int64))
...
class ConvTranspose3d_Q(nn.ConvTranspose3d):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
            padding=0, output_padding = 0, groups=1, bias=True, dilation=1, max_bit=20, F_prior=1e-18):
        super(ConvTranspose3d_Q, self).__init__(in_channels, out_channels, kernel_size, stride,
...
if __name__ == '__main__':
    # debugging code
    conv = Conv3d_Q(3,3,kernel_size=4)
    conv.bit_alloc_w[0][0][0][0][0] = 10
    conv.sync_weight()
