...
class ExperimentPlanner_Interactive_3D(ExperimentPlanner3D_v21):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder, args):
        ...
        if not self.nointeract:
            self.plans_fname = join(self.preprocessed_output_folder,
                                    "nnUNetPlans_interactive_plans_3D.pkl")
        else:
            self.plans_fname = join(self.preprocessed_output_folder,
                                    "nnUNetPlans_interactive_plans_3D_nointeract.pkl")
    ...
    def plan_experiment(self):
        ...
        self.plans_per_stage.append(self.get_properties_for_stage(target_spacing_transposed, target_spacing_transposed,
                                                                  ...
                                                                  len(
                                                                      self.list_of_cropped_npz_files) * self.num_samples,
        ...
        if more:
            ...
            while num_voxels > self.how_much_of_a_patient_must_the_network_see_at_stage0 * architecture_input_voxels_here:
                ...
                new = self.get_properties_for_stage(lowres_stage_spacing_transposed, target_spacing_transposed,
                                                    median_shape_transposed,
                                                    len(self.list_of_cropped_npz_files) * self.num_samples,
                                                    num_modalities, len(all_classes) + 1)
    ...
    def run_preprocessing(self, num_threads, args=None):
        gt_path = join(self.preprocessed_output_folder, "gt_segmentations_only_foreground")

        if not os.path.exists(gt_path):
            shutil.copytree(join(self.folder_with_cropped_data, "gt_segmentations"),
                            join(self.preprocessed_output_folder, "gt_segmentations_only_foreground"))
            list_of_gt = subfiles(gt_path, True, None, ".nii.gz", True)

            foreground_cls = args['foreground_cls']
            all_classes = [0, 1, 2, 3] # for BraTs2020
            background_cls = [i for i in all_classes if i not in foreground_cls]

            for label_path in list_of_gt:
                label = sitk.ReadImage(label_path)
                label = sitk.GetArrayFromImage(label)
                new_label = np.copy(label)
                for c in foreground_cls:
                    new_label[label == c] = 1
                for c in background_cls:
                    new_label[label == c] = 0
                label_img = sitk.GetImageFromArray(new_label)
