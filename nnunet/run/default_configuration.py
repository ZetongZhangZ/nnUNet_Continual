...
def get_nointeract_configuration(network, task, network_trainer, plans_identifier,
                              search_in=(nnunet.__path__[0], "training", "network_training"),
                              base_module='nnunet.training.network_training'):
    assert network in ['2d', '3d_lowres', '3d_fullres', '3d_cascade_fullres'], \
        "network can only be one of the following: \'3d_lowres\', \'3d_fullres\', \'3d_cascade_fullres\'"
    ...
    if network == '2d':
        plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_2D_nointeract.pkl")
    else:
        plans_file = join(preprocessing_output_dir, task, plans_identifier + "_plans_3D_nointeract.pkl")
