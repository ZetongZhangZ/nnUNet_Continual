...
def main():
    ...
    parser.add_argument('--method', type=str, default=None, choices=[None, 'EWC', 'BLIP'],
    ...
    for j in range(start_index,len(training_task)):
        ...
        if not validation_only:
            if j:
                trainer.load_last_dataset_checkpoint()
                if args.method == 'EWC':
                    trainer.EWC_load_previous_episode()

            if args.start_task == task_id:
                if args.continue_training:
                    # -c was set, continue a previous training and ignore pretrained weights
                    trainer.load_latest_checkpoint(trainer.output_folder)
            ...
            if args.pretrained_weights is not None:
                load_pretrained_weights(trainer.network, args.pretrained_weights,ignore_decoder=True)
                # load its own decoder
                trainer.load_decoder(trainer.output_folder,train=False)
            else:
                trainer.load_best_checkpoint(output_folder = trainer.output_folder,train=False)

            trainer.network.eval()
            # predict validation
            # TODO: trainer.transpose_backward is used for validation, need to load for each dataset
